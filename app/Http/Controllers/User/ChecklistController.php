...
{
    private function authorizeChecklistAccess(Checklist $checklist)
    {
        $task = $checklist->task;
        $board = $task->column->board;
        if ($board->user_id !== Auth::id()) {
            abort(403, 'Bạn không có quyền truy cập checklist này.');
        }
    }

    private function authorizeTaskAccessForChecklist(Task $task)
    {
        $board = $task->column->board;
        if ($board->user_id !== Auth::id()) {
            abort(403, 'Bạn không có quyền thao tác checklist cho task này.');
        }
    }

    public function index(Task $task)
    {
        $this->authorizeTaskAccessForChecklist($task);
...
{
    $this->authorizeChecklistAccess($checklist); // Potential 403 if this fails
    ...
    {
        $this->authorizeChecklistAccess($checklist);
        $task = $checklist->task;
