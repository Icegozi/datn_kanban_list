...
var AttachmentManager = (function ($) {
    ...
    function renderExistingAttachment(attachment, container) {
        const fileSizeKB = attachment.file_size ? (attachment.file_size / 1024).toFixed(1) + ' KB' : '';
        const fileIcon = getFileIconClass(attachment.file_name); // Sử dụng attachment.file_name
            ...
            <div class="attachment-item mb-2 p-2 border rounded" data-attachment-id="${attachment.id}">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <a href="${attachment.file_url || '#'}" target="_blank" class="attachment-name font-weight-bold" title="Tải xuống ${attachment.file_name}">${attachment.file_name}</a>
                        ${fileSizeKB ? `<small class="text-muted ml-2">(${fileSizeKB})</small>` : ''}
                        ${attachment.created_at_formatted ? `<small class="text-muted ml-2"> - ${attachment.created_at_formatted}</small>` : ''}
                    </div>
